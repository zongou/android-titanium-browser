...
```sh
if ! test -d .git/modules/vanadium/; then
  git submodule init
...
glient sync and run hooks

this will fetch titanium extension, needed for patch
...
eval "$(cr -c common)"
cd chromium/src
cr -1 patch | while IFS= read -r p; do
  eval "$(cr -c "$p")"
done
```
...
#### patch:crbug.com/406136787:load_unpacked
...
#### patch:crbug.com/445475304:incognito_back
...
#### patch:crbug.com/444024982:api_31
...
$apksigner sign -verbose -ks testkey_untrusted.jks \
  ...
  --key-pass "pass:xrj45yWGLbsO7W0v" \
  --ks-key-alias "alias" \
  --out $chromium_chromium_version-arm64-v8a-signed.apk \
  $chromium_chromium_version-arm64-v8a.apk
```
